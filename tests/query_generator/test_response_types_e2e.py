...
TEST_DATA_FOR_ARRAY_TYPES: dict[
    SupportedPostgresTypeRequiredImports | SupportedPostgresSimpleTypes, TypeTestData
] = {
    SupportedPostgresSimpleTypes.BOOL: TypeTestData(
        query_literal="true",
...
@pytest.mark.parametrize(
    ("data_type_literal", "python_value", "postgres_data_type"),
    [
        (
            TEST_DATA_FOR_ARRAY_TYPES[postgres_data_type].query_literal,
            TEST_DATA_FOR_ARRAY_TYPES[postgres_data_type].expected_python_value,
            postgres_data_type,
        )
        for postgres_data_type in ALL_SUPPORTED_POSTGRES_TYPES
        if postgres_data_type in TEST_DATA_FOR_ARRAY_TYPES
    ],
)
async def test_generate_code_and_execute_for_array_types_in_response_model(
    ...
    match test_case:
        case ArrayTestCases.None_:
            query_literal = f"(null)::{postgres_data_type.value}[]"  # type: ignore[misc]
            expected_python_value = None
        case ArrayTestCases.ArrayOfNone:
            query_literal = f"(ARRAY[null])::{postgres_data_type.value}[]"  # type: ignore[misc]
            expected_python_value = [None]
        case ArrayTestCases.ArrayOfArraysOfNone:
            query_literal = (
                f"(ARRAY[ARRAY[null],ARRAY[null]])::{postgres_data_type.value}[]"  # type: ignore[misc]
            )
            expected_python_value = [[None], [None]]
        case ArrayTestCases.Array:
            query_literal = (
                f"(ARRAY[{data_type_literal}])::{postgres_data_type.value}[]"  # type: ignore[misc]
            )
            expected_python_value = [python_value]
        case ArrayTestCases.ArrayOfArrays:
            query_literal = f"(ARRAY[ARRAY[{data_type_literal}],ARRAY[{data_type_literal}]])::{postgres_data_type.value}[]"  # type: ignore[misc]
            expected_python_value = [[python_value], [python_value]]
        case ArrayTestCases.ArrayOfArraysOfArrays:
            query_literal = f"(ARRAY[ARRAY[ARRAY[{data_type_literal}]],ARRAY[array[{data_type_literal}]]])::{postgres_data_type.value}[]"  # type: ignore[misc]
            expected_python_value = [[[python_value]], [[python_value]]]
        case _:
